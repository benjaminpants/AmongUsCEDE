...
//yes the line #s are wrong no i don't care, also removed the "this only supports unity path" crash because that is incredibly stupid
...
namespace AmongUsCEDE.Utilities
{
	public class WavUtility
	{
		...
		public static AudioClip ToAudioClip(string filePath)
		{
			...
			{
				Debug.LogWarning("the old stuff would error out here but we aint stupid");
			}
		...
		public static AudioClip ToAudioClip(byte[] fileBytes, int offsetSamples = 0, string name = "wav")
		{
			...
			if (!(audioFormat == 1 || audioFormat == 65534)) throw new Exception("wavutil line 49");
		...
		private static float[] Convert8BitByteArrayToAudioClipData(byte[] source, int headerOffset, int dataSize)
		{
			...
			if (!(wavSize > 0 && wavSize == dataSize)) throw new Exception("wavutil line 90");
		...
		private static float[] Convert16BitByteArrayToAudioClipData(byte[] source, int headerOffset, int dataSize)
		{
			int wavSize = BitConverter.ToInt32(source, headerOffset);
			headerOffset += sizeof(int);
			if (!(wavSize > 0 && wavSize == dataSize)) throw new Exception("wavutil line 109");
			...
			if (!(data.Length == convertedSize)) throw new Exception("wavutil line 126");
		...
		private static float[] Convert24BitByteArrayToAudioClipData(byte[] source, int headerOffset, int dataSize)
		{
			int wavSize = BitConverter.ToInt32(source, headerOffset);
			headerOffset += sizeof(int);
			if (!(wavSize > 0 && wavSize == dataSize)) throw new Exception("wavutil line 135");
			...
			if (!(data.Length == convertedSize)) throw new Exception("wavutil line 156");
		...
		private static float[] Convert32BitByteArrayToAudioClipData(byte[] source, int headerOffset, int dataSize)
		{
			int wavSize = BitConverter.ToInt32(source, headerOffset);
			headerOffset += sizeof(int);
			if (!(wavSize > 0 && wavSize == dataSize)) throw new Exception("wavutil line 165");
			...
			if (!(data.Length == convertedSize)) throw new Exception("wavutil line 182");
		...
		public static byte[] FromAudioClip(AudioClip audioClip, out string filepath, bool saveAsFile = true, string dirname = "recordings")
		{
			...
			if (!(bytes.Length == fileSize)) throw new Exception("wavutil line 220");
		...
		private static int WriteFileHeader(ref MemoryStream stream, int fileSize)
		{
			...
			// Validate header
			if (!(count == total)) throw new Exception("wavutil line 256");
		...
		private static int WriteFileFormat(ref MemoryStream stream, int channels, int sampleRate, UInt16 bitDepth)
		{
			...
			// Validate format
			if (!(count == total)) throw new Exception("wavutil line 289");
		...
		private static int WriteFileData(ref MemoryStream stream, AudioClip audioClip, UInt16 bitDepth)
		{
			...
			// Validate header
			if (!(count == total)) throw new Exception("wavutil line 312");

			// Write bytes to stream
			count += WriteBytesToMemoryStream(ref stream, bytes, "DATA");

			// Validate audio data
			if (!(bytes.Length == subchunk2Size)) throw new Exception("wavutil line 318");
		...
		private static byte[] ConvertAudioClipDataToInt16ByteArray(float[] data)
		{
			...
			if (!(data.Length * x == bytes.Length)) throw new Exception("wavutil line 339");
		...
		public static UInt16 BitDepth(AudioClip audioClip)
		{
			...
			if (!(bitDepth == 8 || bitDepth == 16 || bitDepth == 32)) throw new Exception("wavutil line 364");
			return bitDepth;
		}
